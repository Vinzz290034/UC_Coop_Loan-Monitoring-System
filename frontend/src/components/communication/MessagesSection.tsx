...
export default function MessagesSection({ onUnreadCountChange }: MessagesSectionProps) {
  const { user } = useAuth();
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      const data: ContactMessage[] = res.data.data || [];
      setMessages(data);
  ...
  }, [statusFilter, search, onUnreadCountChange]);
  ...
  const handleUpdateStatus = async (id: string, newStatus: string) => {
    ...
    try {
      ...
      setMessages((prev) => {
        const updated = prev.map((m) => (m.id === id ? { ...m, status: newStatus as ContactMessage['status'] } : m));
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      setReplyContent('');
      setMessages((prev) => {
        const updated = prev.map((m) => (m.id === selectedMessage.id ? { ...m, ...updatedData, status: 'resolved' } : m));
        const unread = updated.filter((m) => m.status === 'unread').length;
        if (onUnreadCountChange) onUnreadCountChange(unread);
        return updated;
      });
